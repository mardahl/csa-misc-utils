Workflow blobSync {   
    ...
    $deleteDestBlobs = @();
    $deleteDestBlobs = InlineScript
    {
        $tempArr = @()
        #check to see if the file was removed from source and, if so, remove from destination
        foreach ($blobName in $using:DestBlobsHash.keys) {
            if(-not ($using:SrcBlobs.Name -Contains $blobName)){
                $tempArr += $blobName
            }
        }
        $tempArr;
    ...
    #Remove files in destination that don't match source
    ForEach -parallel -ThrottleLimit 25 ($blobToDelete in $deleteDestBlobs) {
        "  -- blob: $blobToDelete was removed from primary store - deleting it from the backup"
        $workflow:FilesDeleted++  
        $destContext = New-AzStorageContext -StorageAccountName $using:deststorageAccount
        Get-AzStorageBlob -Context $destContext -Container $using:containerName -Blob $blobToDelete | Remove-AzStorageBlob
    }
